...
scatter<-function(x){
  ...
  my.dt <- as.data.table(cbind(abs(x$dot),1))
  x$dot<-my.dt[,row.min:=pmin(V1,V2)]$row.min
  return(acos(x$dot) * (180/pi))
  remove(my.dt)
  # print(Sys.time()-time)#
}
...
normalize_topography<-function(x, res = 5){
  ...
  topo[is.na(slope)]<-NA
  # 
  # density.r<-grid_density(las, topo)
  # # plot(density.r)
  # density.r[density.r<100]<-NA
  # plot(density.r)
  # topo[is.na(density.r)]<-NA
  ...
  # 
  # # 
  # # # density.r[density.r<quantile(getValues(density.r), 0.999)]<-NA
  # # plot(density.r)
  # # 
  # # # topo.smooth[is.na(density.r)]<-NA
  # # plot(topo.smooth)
  # 
  # 
  # # plot(focal(topo, matrix(1,3,3)))
  # # plot(topo)
  # 
  # topo[slope>40]<-NA
  # # topo[slope>40]<-topo.smooth[slope>40]
  # 
  # topo[is.na(slope)]<-NA
  # # topo.smooth[topo.smooth>median(na.omit(getValues(topo.smooth)))+sd(na.omit(getValues(topo.smooth)))]<-NA
  # setMinMax(topo)
  # plot(topo)
  # 
  # topo.df<-as.data.frame(rasterToPoints(topo))
  ...
  # ground<-classify_ground(topo.las, pmf(ws, th), last_returns = FALSE)
  # # plot(ground)
  # # topo.las.r<-grid_terrain(ground, res=res(r)[1], knnidw(k = 21))
  # topo.las.r<-grid_terrain(ground, r, kriging(k = 10))
  # 
  # plot(topo.las.r)
  # # topo.las.r[is.na( topo.las.r)]<-focal(topo.las.r, matrix(1,3,3))[is.na( topo.las.r)]
  # # plot(is.na(topo.las.r))
  # 
  # r.p<-buffer(topo.las.r, width=1)
  # # plot(r.p)
  # # r.p<-topo.las.r
...
rfPrep<-function(c2c.file){
  ...
  # for(ii in 1:length(dat.temp)){
  #   if (class(dat.temp[[ii]])=="character"){
  #     dat.temp[[ii]]<- as.numeric( dat.temp[[ii]] )
  #   } 
  # } 
...
TLSLeAF<-function(input_file,
  ...
  if(!file.exists(class.file.name)&
    ...
    # 
    # fwrite(dat.rf, file = class.file.name, sep = " ", row.names = FALSE)
    # remove(dat.rf)
    # gc()
    # Sys.sleep(10)
